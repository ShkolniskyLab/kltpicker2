...
def parse_args(has_cupy):
    ...
    parser.add_argument('--save-asocem', action='store_true', help='Choose to save ASOCEM mrc.', default=False)
    parser.add_argument('--asocem-downsample', help='Use to set asocem downsample size.', default=600, type=check_positive_int)
    parser.add_argument('--asocem-area', help='Use to set asocem covariance area size.', default=5, type=check_positive_odd_int)
...
def check_positive_odd_int(value):
    ivalue = int(value)
    if ivalue >= 0:
        if ivalue % 2 == 1:
            return ivalue
    raise argparse.ArgumentTypeError("%s is an invalid positive odd int value" % value)
...
def get_args(has_cupy):
    ...
    if has_cupy:
        ...
        no_gpu = 1
        gpu_indices = []
    return input_dir, output_dir, particle_size, num_particles_to_pick, num_noise_to_pick, use_asocem, save_asocem_masks, asocem_downsample, asocem_area, no_gpu, gpu_indices, verbose, max_processes, only_do_unfinished
