...
def fftconvolve2d_many(x, y, cpu=True):
    """
    Convolve two 2-dimensional arrays using FFT, utilizing CuPy.
    The convolution is in valid mode, meaning that the output consists only of
    those elements that do not rely on the zero-padding.
    ...
    x_fft = xp.fft.fft2(x, s=(zn, zm))
    out = xp.zeros((num_filters, end_n - start_n, end_m - start_m))
    for i, curr_filt in enumerate(y):
        z = xp.fft.ifft2(x_fft * xp.fft.fft2(curr_filt, s=(zn, zm))).real
        out[i] = z[start_n:end_n, start_m:end_m]
    return out


def compute_log_test_n(noise_mc, qp, D, mu):
    """
    Convolve two 2-dimensional arrays using FFT, utilizing CuPy.
    The convolution is in valid mode, meaning that the output consists only of
    those elements that do not rely on the zero-padding.
    ...
    ## Changing shape of y
    num_of_func = len(qp)
    patch_size_func = int(np.sqrt(qp.shape[1]))
    qp_filters = []
    for i in range(num_of_func):
        qp_tmp = np.reshape(qp[i, :], (patch_size_func, patch_size_func))
        qp_filters.append(np.sqrt(D[i]) * np.flip(qp_tmp))

    qp_filters = cp.asarray(qp_filters)
    noise_mc = cp.asarray(noise_mc.T)

    # Getting shapes
    xn, xm = noise_mc.shape
    num_filters, yn, ym = num_of_func, patch_size_func, patch_size_func
    zn = xn + yn - 1
    zm = xm + ym - 1
    valid_n = xn - yn + 1
    valid_m = xm - ym + 1
    ...
    # apply filter, summing squares and reducing by mu
    x_fft = cp.fft.fft2(noise_mc, s=(zn, zm))
    log_test_mat = cp.full((end_n - start_n, end_m - start_m), -mu)
    for i, curr_filt in enumerate(qp_filters):
        z = cp.fft.ifft2(x_fft * cp.fft.fft2(curr_filt, s=(zn, zm))).real
        log_test_mat += z[start_n:end_n, start_m:end_m] ** 2

    # Computing log_test_n and moving it no numpy
    neigh = cp.ones((patch_size_func, patch_size_func))
    log_test_n = fftconvolve2d_gpu(log_test_mat, neigh)
    log_test_n = cp.asnumpy(log_test_n)

    return log_test_n


def unique_tol(x, tol):
    tol *= np.max(np.abs(x))
    ...
    curr_idx = 0
    while curr_idx < len(x):
        curr_unique = sorted_x[curr_idx]
        last_index = np.searchsorted(sorted_x, curr_unique + tol)
        pos[sorted_idx[curr_idx:last_index]] = len(uniques)
        uniques.append(curr_unique)
        curr_idx = last_index
    return np.array(uniques), pos
